...
class AcconeerIQNode(Node):
    ...
    def publish_range_profile(self, distances, amplitude, variance, phase_std):
        ...
        # Fields (x, y, z, amplitude, variance, phase_std)
        fields = [
        ...
        pc2_data = [struct.pack('<ffffff', *p) for p in points]
        ...
        msg = PointCloud2(
            ...
            data=b''.join(pc2_data)
        )
        
        self.pcl_pub.publish(msg)
        self.get_logger().info(f"Published range profile: {len(points)} bins", throttle_duration_sec=1.0)
